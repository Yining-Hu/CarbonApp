...
router.post('/log/sale',
    ...
    (request, response) => {
        ...
        if (!paramerrors.isEmpty()) {
            ...
            sfinstance.then(value => {
                value.methods.logSale(saleid,quantity,storageid,datetime).send({from: request.body.bcacc, gas: gas})
                .then((result) => {
                    console.log(result);
...
router.post('/update/order',
    ...
    validator.check("orderstatus").isInt().withMessage("Input should be an interger in the range [0,3]."),
    validator.check("gas").exists().withMessage("Input should contain field 'gas'."),
    validator.check("gas").isInt(),
